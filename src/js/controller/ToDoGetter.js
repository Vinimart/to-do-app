...
class ToDoGetter {
  ...
  add() {
    ...
    if (this.name.value) {
      ...
      // Envia os parâmetros para a lista de objetos que armazenam as props do componente to-do
      this.list.push({
  ...
  restoreToDo() {
    ...
    this.trashList = this.list.filter((e) => e.trash === true);
  ...
  loadList(array) {
    // Passa para o construtor do componente as propriedades dos to-dos existentes no localStorage
    array.forEach((item) => {
